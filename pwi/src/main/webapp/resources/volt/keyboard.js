(function(f) {
	...
	function l() {
		...
		this.mainDiv = f('<div id="' + this._mainDivId
	...
	f
			.extend(
					l.prototype,
					{
						...
						HALF_SPACE : c,
						ENTER : g,
						qwertyAlphabetic : [ "qwertyuiop", "asdfghjkl",
								"zxcvbnm" ],
						qwertyLayout : [ "!@#$%^&*()_=" + c + h + e,
								c + "`~[]{}<>\\|/" + h + "789",
								"qwertyuiop'\"" + c + "456",
								c + "asdfghjkl;:" + h + "123",
								h + "zxcvbnm,.?" + h + c + "-0+",
						...
						_attachKeypad : function(p, m) {
							var o = (p.nodeName.toLowerCase() != "input" && p.nodeName
									.toLowerCase() != "textarea");
							var n = {
								_inline : o,
								_mainDiv : (o ? f('<div class="'
						...
						_connectKeypad : function(r, q) {
							...
							if (!q._inline) {
								...
								if (u == "focus" || u == "both") {
									s.focus(this._showKeypad).keydown(
											this._doKeyDown)
								}
								if (u == "button" || u == "both") {
									...
									var n = f(this._get(q, "buttonImageOnly") ? f('<img src="'
											+ v
											+ '" alt="'
											+ o
											+ '" title="'
							...
							s.addClass(this.markerClassName).bind(
									"setData.keypad", function(x, w, y) {
										q.settings[w] = y
						...
						_destroyKeypad : function(o) {
							...
							m.siblings("." + this._appendClass).remove().end()
									.siblings("." + this._triggerClass)
									.remove().end().prev(
											"." + this._inlineEntryClass)
									.remove();
							m.empty().unbind("focus", this._showKeypad)
									.removeClass(this.markerClassName).attr(
											"readonly", n.saveReadonly);
						...
						_enableKeypad : function(o) {
							...
							if (p == "input" || p == "textarea") {
								o.disabled = false;
								n.siblings("button." + this._triggerClass)
										.each(function() {
											this.disabled = false
										}).end().siblings(
						...
						_disableKeypad : function(p) {
							...
							if (s == "input" || s == "textarea") {
								p.disabled = true;
								o.siblings("button." + this._triggerClass)
										.each(function() {
											this.disabled = true
										}).end().siblings(
												"img." + this._triggerClass)
								...
								if (s == "div" || s == "span") {
									...
									o.prepend('<div class="'
											+ this._disableClass
											+ '" style="width: '
											+ n.outerWidth() + "px; height: "
											+ n.outerHeight() + "px; left: "
											+ (r.left - q.left) + "px; top: "
											+ (r.top - q.top) + 'px;"></div>');
									var m = f.data(p, j);
									m._mainDiv.find("button").attr("disabled",
											"disabled")
						...
						_showKeypad : function(r) {
							r = r.target || r;
							if (f.keypad._isDisabledKeypad(r)
									|| f.keypad._lastField == r) {
								return
							}
							var o = f.data(r, j);
							...
							p = (p == "normal" && f.ui && f.ui.version >= "1.8" ? "_default"
									: p);
							var n = function() {
								f.keypad._keypadShowing = true;
								var t = f.keypad._getBorders(o._mainDiv);
								o._mainDiv.find(
										"iframe." + f.keypad._coverClass).css({
									left : -t[0],
									top : -t[1],
							...
							if (f.effects && f.effects[m]) {
								o._mainDiv.show(m, f.keypad._get(o,
										"showOptions"), p, n)
							} else {
						...
						_updateKeypad : function(m) {
							...
							m._mainDiv.empty().append(this._generateHTML(m))
									.find("iframe." + this._coverClass).css({
										left : -n[0],
										top : -n[1],
							...
							m._mainDiv
									.removeClass()
									.addClass(
											this._get(m, "keypadClass")
													+ (this._get(m, "isRTL") ? " keypad-rtl"
															: "")
							...
							if (o) {
								o.apply((m._input ? m._input[0] : null), [
										m._mainDiv, m ])
							}
						},
						_getBorders : function(m) {
							...
							return [ parseFloat(n(m.css("border-left-width"))),
									parseFloat(n(m.css("border-top-width"))) ]
						},
						_checkOffset : function(p, o, n) {
							var r = p._input ? this._findPos(p._input[0])
									: null;
							...
							if ((f.browser.msie && parseInt(f.browser.version,
									10) < 7)
									|| f.browser.opera) {
								var m = 0;
								p._mainDiv
							...
							if (this._get(p, "isRTL")
									|| (o.left + p._mainDiv.outerWidth() - t) > u) {
								o.left = Math
										.max(
												(n ? 0 : t),
												r[0]
														...
														- p._mainDiv
																.outerWidth()
														- (n && f.browser.opera ? document.documentElement.scrollLeft
																: 0))
							} else {
								o.left -= (n ? t : 0)
							}
								...
								o.top = Math
										.max(
												...
												r[1]
														...
														- p._mainDiv
																.outerHeight()
														- (n && f.browser.opera ? document.documentElement.scrollTop
																: 0))
							} else {
								o.top -= (n ? s : 0)
							}
						...
						_hideKeypad : function(q, p) {
							...
							if (this._keypadShowing) {
								...
								p = (p == "normal" && f.ui
										&& f.ui.version >= "1.8" ? "_default"
										: p);
								var n = this._get(o, "showAnim");
								if (f.effects && f.effects[n]) {
									o._mainDiv.hide(n, this._get(o,
											"showOptions"), p)
								} else {
									o._mainDiv[(n == "slideDown" ? "slideUp"
											: (n == "fadeIn" ? "fadeOut"
													: "hide"))](n ? p : "")
								}
							}
							...
							if (m) {
								m.apply((o._input ? o._input[0] : null), [
										o._input.val(), o ])
							}
							if (this._keypadShowing) {
								this._keypadShowing = false;
						...
						_checkExternalClick : function(m) {
							...
							var n = f(m.target);
							if (!n.parents().andSelf().is(
									"#" + f.keypad._mainDivId)
									&& !n.hasClass(f.keypad.markerClassName)
									&& !n.parents().andSelf().hasClass(
											f.keypad._triggerClass)
									&& f.keypad._keypadShowing) {
								f.keypad._hideKeypad(null, "")
							}
						},
						...
						_backValue : function(o) {
							...
							if (p.setSelectionRange) {
								m = (o._input.attr("readonly")
										|| o._input.attr("disabled") ? m : [
							...
							this._setValue(o, (n.length == 0 ? "" : n.substr(0,
									m[0] - 1)
									+ n.substr(m[1])), m[0] - 1);
						...
						insertValue : function(n, o) {
							...
							if (q.setSelectionRange) {
								m = (n.attr("readonly") || n.attr("disabled") ? m
										: [ q.selectionStart, q.selectionEnd ])
							} else {
								if (q.createTextRange) {
									m = (n.attr("readonly")
											|| n.attr("disabled") ? m : this
											._getIERange(q))
								}
							}
						...
						_getIETextRange : function(o) {
							...
							var m = (n ? o.createTextRange() : document.body
									.createTextRange());
							if (!n) {
						...
						_notifyKeypress : function(o, m) {
							...
							if (n) {
								n.apply((o._input ? o._input[0] : null), [ m,
										o._input.val(), o ])
							}
						},
						_get : function(n, m) {
						...
						_generateHTML : function(r) {
							...
							var s = (!n ? "" : '<div class="keypad-prompt">'
									+ n + "</div>");
							...
							s += '<div style="clear: both;"></div>'
									+ (!r._inline
											&& f.browser.msie
											&& parseInt(f.browser.version, 10) < 7 ? '<iframe src="javascript:false;" class="'
											+ f.keypad._coverClass
											+ '"></iframe>'
											: "");
							s = f(s);
							...
							}).end().filter(".keypad-close").click(
									function() {
										f.keypad._curInst = (u._inline ? u
												: f.keypad._curInst);
										f.keypad._hideKeypad()
									}).end().filter(".keypad-shift").click(
									function() {
										f.keypad._shiftKeypad(u)
									}).end().filter(".keypad-enter").click(
									function() {
										f.keypad._selectValue(u, f.keypad.EN)
									}).end().not(".keypad-clear").not(
									".keypad-back").not(".keypad-close").not(
									".keypad-shift").not(".keypad-enter")
									.click(
											function() {
												f.keypad._selectValue(u,
														f(this).text())
											});
							return s
						},
	...
	f.fn.keypad = function(n) {
		...
		if (n == "isDisabled") {
			return f.keypad["_" + n + "Keypad"].apply(f.keypad, [ this[0] ]
					.concat(m))
		}
		return this.each(function() {
			typeof n == "string" ? f.keypad["_" + n + "Keypad"].apply(f.keypad,
					[ this ].concat(m)) : f.keypad._attachKeypad(this, n)
		})
	};
	f.keypad = new l();
	f(function() {
		f(document.body).append(f.keypad.mainDiv).mousedown(
				f.keypad._checkExternalClick)
	})
})(jQuery);
PrimeFaces.widget.Keyboard = PrimeFaces.widget.BaseWidget.extend({
	init : function(a) {
		...
		if (this.cfg.layoutTemplate) {
			this.cfg.layout = PrimeFaces.widget.KeyboardUtils
					.createLayoutFromTemplate(this.cfg.layoutTemplate)
		} else {
			this.cfg.layout = PrimeFaces.widget.KeyboardUtils
					.getPresetLayout(this.cfg.layoutName)
		}
...
PrimeFaces.widget.KeyboardUtils = {
	layouts : {
		qwertyBasic : [
				...
				$.keypad.SPACE + $.keypad.qwertyAlphabetic[2] + $.keypad.SHIFT
						+ $.keypad.BACK ],
		qwerty : $.keypad.qwertyLayout,
		alphabetic : [
				"abcdefghij" + $.keypad.CLOSE,
				"klmnopqrst" + $.keypad.CLEAR,
				"uvwxyz" + $.keypad.SPACE + $.keypad.SPACE + $.keypad.SHIFT
						+ $.keypad.BACK ]
	},
	controls : {
		close : $.keypad.CLOSE,
